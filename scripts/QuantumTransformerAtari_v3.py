"""
Quantum Time-Series Transformer for Atari Environments — v3
=============================================================
This script implements a quantum transformer-based RL agent for classic Atari games
using hybrid CNN + Quantum architecture.

v3 improvements over v1/v2 (QuantumTransformerAtari.py):
  1. Sinusoidal Positional Encoding (Vaswani et al., 2017)
     — gives the model temporal awareness of frame ordering
  2. 2pi angle scaling: (sigmoid - 0.5) * 2pi for full rotation range [-pi, pi]
     — centered at 0 to avoid barren plateau at theta=pi
     — utilises the full quantum gate rotation range (v1 only used [0, 1])
  3. Separate CNN instances for online/target networks
     — v1/v2 shared one CNN, causing target's requires_grad=False to freeze
       the online CNN's gradients
...
Architecture:
  ...
  -> CNN Feature Extractor -> + Sinusoidal PE -> Quantum Transformer ([-π,π]) -> Q-values
...
def get_args():
    parser = argparse.ArgumentParser(
        description='Quantum Transformer v3 for Atari Games'
    )
...
RUN_ID = f"QTransformerV3_{env_name_clean}_Q{args.n_qubits}_L{args.n_layers}_D{args.degree}_Run{args.log_index}"
...
# ================================================================================
# QUANTUM TIME-SERIES TRANSFORMER (v3: + Positional Encoding + 2pi scaling)
# ================================================================================
class QuantumTSTransformerRL(nn.Module):
    """Quantum Time-Series Transformer for Atari (v3).

    v3 changes:
      - Sinusoidal Positional Encoding (Vaswani et al., 2017) injected before
        feature projection, giving temporal awareness of frame ordering.
      - 2pi angle scaling: (sigmoid - 0.5) * 2pi for [-pi, pi] rotation range
        centered at 0 to avoid barren plateau (v1 used [0, 1]).
    """
...
class QuantumAtariAgent:
    """DQN agent for Atari using quantum transformer."""
...
def train():
    ...
    print("\n" + "="*80)
    print(f"QUANTUM TRANSFORMER V3 - {args.env}")
    print("="*80)
    print(f"\n📊 Configuration:")
    ...
    print(f"  • v3 Features:   Sinusoidal PE + 2pi angle scaling")
